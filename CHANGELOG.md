...
- 新增 Bark 推送通知：在「设置 > 常规 > Bark 推送」配置服务端地址（默认官方 `api.day.app`，支持自建）、Device Key、铃声与通知分组（可留空），可发送测试推送；提供「非锁屏时跳过推送」开关。
- Bark 推送携带 `id` 覆盖参数（按 Provider + 模型 + 事件类型稳定生成），同一类事件的新推送会覆盖手机上的旧通知，不再堆积历史提醒。
- 通知按模型合并：一次刷新中同一个模型触发的多个事件（如 5 小时额度恢复 + 周额度耗尽）合并为一条通知，通知渠道取该模型全部事件配置的并集；系统通知与 Bark 行为一致。
