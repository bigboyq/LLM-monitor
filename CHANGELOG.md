...
- 额度通知细分为四类事件（5 小时额度恢复 / 耗尽、周额度恢复 / 耗尽）。有窗口额度的 Provider（ChatGPT、GLM、minimax、Antigravity）在各自设置页可独立配置每类事件的通知渠道：不通知、系统通知、Bark + 系统通知；默认恢复 → 系统通知、耗尽 → 不通知，与既有行为一致。恢复判定为「回升超过 5 个百分点，或回到 98% 以上且严格回升」，耗尽为剩余百分比下穿 0.01% 的边沿触发；系统通知标题按事件类型显示「额度已用完 / 已恢复 / 额度提醒」。DeepSeek 为余额口径、无时间窗口，暂不接入。
