...
- 新增 Bark 推送通知：在「设置 > 常规 > Bark 推送」配置服务端地址（默认官方 `api.day.app`，支持自建，保留反向代理子路径 base path）、Device Key（掩码输入）、铃声与通知分组（可留空），可发送测试推送（与正式推送共用规范化、URL 构造与锁屏策略）；提供「非锁屏时跳过推送」开关。仅允许 https 与本机 http 调试地址。
- Bark 推送携带 `id` 覆盖参数（按 Provider + 模型稳定生成，不随事件组合变化），同一模型的新推送会覆盖手机上的旧通知，不再堆积历史提醒（需 Bark v1.5.2+ / bark-server v2.2.5+）。
- 通知按模型合并且正文按渠道过滤：一次刷新中同一模型的多个事件合并为一条通知，但每个渠道只包含该渠道已启用的事件文案（「不通知」的事件不进入任何通知）；系统通知与 Bark 行为一致。Bark 发送经串行队列：同一模型 60 秒冷却、积压上限、5xx / 网络瞬断自动重试一次。
