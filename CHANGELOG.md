...
- 新增 Bark 推送通知：在「设置 > 常规 > Bark 推送」配置服务端地址（默认官方 `api.day.app`，支持自建，保留反向代理子路径 base path）、Device Key（掩码输入）、铃声与通知分组（可留空），可发送测试推送（与正式推送共用规范化、请求构造与锁屏策略）；提供「非锁屏时跳过推送」开关。仅允许 https 与本机 http 调试地址。
...
- 额度通知细分为四类事件（5 小时额度恢复 / 耗尽、周额度恢复 / 耗尽）。有窗口额度的 Provider（ChatGPT、GLM、minimax、Antigravity）在各自设置页可独立配置每类事件的通知渠道：不通知、系统通知、Bark + 系统通知；默认恢复 → 系统通知、耗尽 → 不通知，与既有行为一致。DeepSeek 为余额口径、无时间窗口，暂不接入。
