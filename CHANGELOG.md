...
- 设置页「图标主题」选择器中「App 图标」的预览图与主面板左上角 header 图标统一改为 App 图标设计稿（`llm-quota-730-2-dark.svg`，与 App 实际图标同源），尺寸与原显示保持一致；设计稿加载失败时回退到原现生成图 / 系统符号。Icon Duo 及系统符号样式的 picker 预览仍为现生成。
