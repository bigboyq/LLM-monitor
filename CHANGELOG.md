...
- 主面板 header 标题旁新增「N 个应用正在阻止休眠」secondary 提示：悬浮展开阻止休眠的第三方应用清单（进程名 · PID · 断言类型 · 已持续时长，与设置页节能 Tab 检查项一同源），无阻止应用时自动隐藏；行文案与设置页共用同一格式化，避免两处漂移。
- 状态栏新增「Icon Duo」图标方案：完整保留 1.9.0 重写后的额度仪表盘布局——左右弧线分别显示 5 小时与周额度，中心扇形按所有套餐中最低的 5 小时剩余比例动态展开 0～360°，底部三个套餐健康点与顶部节能/睡眠状态点。

### Changed

- 打开主面板时即时刷新睡眠健康度评估：footer「节能」健康灯与 header「N 个应用正在阻止休眠」提示在面板打开瞬间反映最新断言状态，无需等待下一次调度健康边界或进入设置页；刷新由面板窗口 become key 事件驱动，不引入常驻定时器。
- 设置页「图标主题」选择器中「App 图标」的预览图改为直接使用设计稿 SVG（`llm-quota-730-2-dark.svg`，与经典图标同源），不再用额度全满的示例指标现生成；主面板左上角 header 图标改为完整 App 图标（`icon-master.png`，含圆角底与渐变背景），尺寸与原显示保持一致；两者资源加载失败时分别回退到现生成图 / 系统符号。Icon Duo 及系统符号样式的 picker 预览仍为现生成。
- 「App 图标」（quotaLogo）恢复为上一版（1.6.0–1.8.x）的经典实现与样式：外环周额度、内环 5 小时额度自 12 点钟逆时针充盈（实线到最低剩余量、2-4 刻度虚线到平均值），中心水位杯高度映射 5 小时最低剩余、水体颜色跟随警报状态；缺失窗口按满环显示（与旧版一致）。已在设置中选择 App 图标的用户升级后自动回到经典样式，新布局请切换到「Icon Duo」。
- 修正水位警报判定的缺失数据语义：启用套餐完全没有 5 小时窗口时，水位警报按满量参与判定（此前会被误判为耗尽红色）。该判定仅影响经典 App 图标的水体颜色。

### Fixed

- 修复常驻内存随刷新次数持续增长：Antigravity 抓取器此前每次构造都新建一个带 delegate 的 URLSession 且从不释放（每轮额度刷新、每次用量 reconcile、认证探测与状态重建各泄漏一份，默认配置每小时累积数十个），现改为进程级共享单个 session，并消除了每轮 reconcile 中为常量返回值构造整个抓取器的调用。长时间运行内存不再单调上涨。
- 重新生成 `AppIcon.icns` 使其与 9-17 更新的 `icon-master.png` 同步：icns 此前停留在 9-08 版本，旧 macOS 的图标回退路线会显示旧版 App 图标。
- 设置页「图标主题」预览图加入缓存：拖动刷新间隔滑杆或取色器时不再每个 tick 重建 SVG 与 NSImage（主线程每张 1-3ms）。
- 主面板 header 的完整 App 图标改为持有 128px 降采样位图（约 64KB）而非 1024px 解码位图（约 4MB 常驻），24pt 显示视觉无损。
- dsh 本地用量扫描的注入式解压路径补齐输入/输出字节上限（生产路径本就流式，此为防御性加固）。
...
- 国内 / 海外 HTTP 超时分为两档（`HTTPTimeouts`）：minimax / GLM / DeepSeek 等国内额度端点收紧到 10 秒（实测 600+ 样本 p99 ≤ 0.8s、最慢 ~1.8s，10s 仍可扛住 TCP SYN 重传），codex 海外端点 15 秒；网络抖动时 UI 不会长时间卡在刷新状态。
- 刷新失败不再指数退避：后台刷新本就是固定间隔，拉长重试间隔只会推迟恢复；失败后改为按原定间隔随下一定时周期重试（`.deferred` 1 秒短重试早醒机制不变）。
- 退役配置键 `statusBarIndicatorMode`：该键自 1.9.0 起已无实际作用，现从代码中移除；config.json 中残留的旧值会被安全忽略（不影响配置解析），下次保存配置时自动消失。

### Fixed

- 打开设置窗口等操作不再使应用图标出现在 Dock 中：应用全程固定 accessory 激活策略，保持纯菜单栏形态。

## [1.9.0] - 2026-09-16

### Added

- 状态栏新增额度仪表盘布局：左侧显示 5 小时额度、右侧显示周额度；中心扇形按当前最紧张额度的剩余百分比在 0°～360° 动态展开；底部最多三个状态点按红 > 黄 > 绿排序汇总模型健康度；顶部状态点显示节能/睡眠健康度。
- 本地用量扫描统一接入 FSEvents 与源生命周期管理，文件变化可增量唤醒对应扫描器，并保留首次、跨日及配置变化时的完整校准扫描。

### Changed

- Provider 常规刷新、reset-time 补刷新与睡眠健康检查统一由可取消的 deadline driver 调度，合并相同截止时间的重复唤醒。
- 菜单窗口按真实内容高度与当前屏幕可用区域动态调整；Footer 保持 360pt 面板宽度并固定为单行显示。

### Fixed

- 统一状态栏仪表盘的健康度阈值和缺失数据语义，避免剩余时间因子或无窗口模型错误改变图标颜色。
- 收紧本地文件事件过滤、指纹校验与取消时序，避免无关目录事件触发扫描或旧任务覆盖新结果。
- 构建脚本改为读取 SwiftPM 本次构建实际产物目录，避免残留的旧 release 二进制或资源包被重新打入新 App。

## [1.8.0] - 2026-09-14

### Added

- 新增「节能」系统睡眠健康度诊断与防休眠管理模块：
  - 主面板底部 Footer 新增「节能」快捷按钮（带三色健康状态指示灯），单击就地切换「防止休眠」模式（纯内存断言，应用退出或重启后自动复位，安全无后患）。
  - 设置窗口新增「节能」Tab（powersleep 图标）：状态卡片全量展示睡眠健康度状态、阻止系统休眠的第三方违规应用列表（进程名、PID、断言类型与实时持续时长），以及 AC 电源休眠为 0 时的配置修复指引。
  - 电源参数矩阵表：结构化展示 `sleep`、`womp`、`tcpkeepalive`、`powernap`、`displaysleep` 在电源适配器（AC）与电池供电下的当前取值及 GUI / CLI 操作指引。
  - 直通 IOKit 原生 C API（`IOPMCopyPMPreferences` 与 `IOPMCopyAssertionsByProcess`）：亚毫秒级直接读取内核电源配置字典与活动断言，彻底避免子进程调用开销与字符解析格式风险。
  - 调度归一化：移除独立 60 秒定时器，睡眠健康度评估全面并入 `LocalUsage` 循环节拍与全量手动刷新。

## [1.7.0] - 2026-09-13

### Added

- 新增 Bark 推送通知：在「设置 > 常规 > Bark 推送」配置服务端地址（默认官方 `api.day.app`，支持自建，保留反向代理子路径 base path）、Device Key（掩码输入）、铃声、通知分组与消息有效期 TTL（秒；留空或 0 = 不携带参数，> 0 时过期后手机端自动删除该消息），可发送测试推送（与正式推送共用规范化、请求构造与屏幕跳过策略）；提供「人在电脑前时跳过推送」开关——屏幕亮着且未锁屏才跳过 Bark，显示器休眠或已锁屏（人不在）都正常推送。仅允许 https 与本机 http 调试地址。
- Bark 推送采用官方文档的 POST JSON 形式（`POST {server}/{key}`，标题/正文/group/id 放 JSON body）：规避 GET 路径的 URL 编码与 2048 长度限制，正文里的特殊字符无需转义。
- Bark 推送携带 `id` 覆盖参数（按 Provider + 模型稳定生成，不随事件组合变化），同一模型的新推送会覆盖手机上的旧通知，不再堆积历史提醒（需 Bark v1.5.2+ / bark-server v2.2.5+）。
- 通知按模型合并且正文按渠道过滤：一次刷新中同一模型的多个事件合并为一条通知，但每个渠道只包含该渠道已启用的事件文案（「不通知」的事件不进入任何通知）；系统通知与 Bark 行为一致。系统通知与 Bark 发送均有同模型 60 秒冷却；Bark 队列串行发送、积压超限时按模型合并保留最新一条、5xx / 网络瞬断自动重试一次，App 退出时取消未完成的推送，发送失败不占用冷却窗口。
- 额度通知细分为四类事件（5 小时额度恢复 / 耗尽、周额度恢复 / 耗尽）。有窗口额度的 Provider（ChatGPT、GLM、minimax、Antigravity）在各自设置页可独立配置每类事件的通知渠道：不通知、系统通知、Bark + 系统通知；渠道默认与既有行为一致（恢复 → 系统通知、耗尽 → 不通知）；恢复判定本身已从「任意 ≥0.01pp 回升」调整为下述新规则。恢复判定为「回升超过 5 个百分点，或回到 98% 以上且严格回升」，耗尽为剩余百分比下穿 0.01% 的边沿触发；系统通知标题按事件类型显示「额度已用完 / 已恢复 / 额度提醒」。DeepSeek 为余额口径、无时间窗口，暂不接入。
- 额度事件的检测基线持久化到 `notification-state.json`（跨重启连续）：App 停机期间发生的耗尽 / 恢复事件，重启后第一次刷新即可补报，而不是静默重建基线漏掉；Provider 转入未配置状态时基线随之清空。
