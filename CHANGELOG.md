...
- 新增 Bark 推送通知：在「设置 > 常规 > Bark 推送」配置服务端地址（默认官方 `api.day.app`，支持自建，保留反向代理子路径 base path）、Device Key（掩码输入）、铃声与通知分组（可留空），可发送测试推送（与正式推送共用规范化、请求构造与屏幕跳过策略）；提供「人在电脑前时跳过推送」开关——屏幕亮着且未锁屏才跳过 Bark，显示器休眠或已锁屏（人不在）都正常推送。仅允许 https 与本机 http 调试地址。
