...
/// 单日 5 类 token 用量（同构 `MinimaxDailyUsage`）。
///
/// 与 minimax 的关键区别：opencode 的 `tokens.reasoning` 是**账单原生值**（GLM-5.2 有
/// 真实 reasoning），不需要 minimax 那套 `thinking_content` 字符分摊。
struct OpencodeDailyUsage: Equatable, Codable, Sendable, Identifiable {
    let dayStart: Date
    let inputTokens: Int
    ...
    /// 跨 source 合并（sum，scanner 内部用）。饱和加法，负数归零。
    static func + (lhs: OpencodeDailyUsage, rhs: OpencodeDailyUsage) -> OpencodeDailyUsage {
        OpencodeDailyUsage(
...
extension OpencodeDailyUsage: DailyUsageAddable {
    init(dayStart: Date) {
        self.init(
            dayStart: dayStart,
            inputTokens: 0,
    ...
    func withDayStart(_ date: Date) -> OpencodeDailyUsage {
        OpencodeDailyUsage(
            dayStart: date,
