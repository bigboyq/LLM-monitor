...
- 设置 → 常规：修改刷新间隔、状态栏图标、状态圆点显示开关和开机自启动。状态栏图标支持系统符号和 App 图标；App 图标是实时额度仪表盘，左右分别显示 5 小时与周额度弧线，中间扇区显示所有套餐中最低的 5 小时剩余额度（没有 5 小时窗口时回退到周额度），底部最多显示 3 个模型健康度点，顶部圆点显示节能/睡眠健康度。
