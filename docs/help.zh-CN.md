...
- 设置 → 常规：修改刷新间隔、状态栏图标、状态圆点显示开关和开机自启动。状态栏图标支持系统符号和两种自包含仪表盘样式：「App 图标」为经典双环水位样式——外环周额度、内环 5 小时额度（实线充盈到最低剩余量、刻度虚线延伸到平均值，逆时针绘制），中心水位高度映射 5 小时最低剩余、水体颜色跟随警报状态（缺失窗口按满环显示，与旧版一致）；「Icon Duo」为额度仪表盘——左右弧线显示 5 小时与周额度，中心扇形显示所有套餐中最低的实际可用额度（每个套餐取 5 小时剩余与「周剩余 × 周倍率」中较小者，与卡片分段条同口径），底部最多显示 3 个模型健康度点（与卡片一致的时间感知阈值判定，GLM 高峰时段对应点不低于黄色），顶部圆点显示节能/睡眠健康度。
...
- **三色健康度指示**：
  ...
  - 🔴 **红色**：已手动开启「防止休眠」模式。
- **状态刷新时机**：打开主面板、进入「设置 → 节能」、系统唤醒与调度健康边界（如高峰时段切换）时即时重新评估；footer 健康灯与 header 阻止休眠提示始终反映最新状态，无需手动刷新。
...
| 本地 scanner 缓存 | `~/Library/Application Support/LLM-monitor/token-monitor/{provider}.json` |
...
4. 各客户端的原始数据库不会被删除；`token-monitor` 缓存文件可单独移除，并可由应用重新生成。
