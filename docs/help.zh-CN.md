...
- 设置 → 常规：修改刷新间隔、状态栏图标、状态圆点显示开关和开机自启动。状态栏图标支持系统符号和两种自包含仪表盘样式：「App 图标」为经典双环水位样式——外环周额度、内环 5 小时额度（实线充盈到最低剩余量、刻度虚线延伸到平均值，逆时针绘制），中心水位高度映射 5 小时最低剩余、水体颜色跟随警报状态（缺失窗口按满环显示，与旧版一致）；「Icon Duo」为额度仪表盘——左右弧线显示 5 小时与周额度，中心扇形显示所有套餐中最低的 5 小时剩余额度（没有 5 小时窗口时回退到周额度），底部最多显示 3 个模型健康度点，顶部圆点显示节能/睡眠健康度。
