...
## 〇、实施结果（2026-08-18 当日落地，本轮追加性能、缺陷与审计修复，375→376→374→376→377 测试全绿）

源码 92 → 99 文件 / 23,679 → 23,365 行（**-314 行 / -1.3%**）；测试 24 文件 /
12,578 → 12,783 行（+205 行，主要是新增回归覆盖与完整文档注释）。行数降幅低于原估的
-1,900，主要因为：阶段 1 的
watcher 重挂载/退避是新增防御代码（+137）；阶段 2/3 为保住全部既有测试表面保留了
per-scanner 静态函数（原估算假设删掉）；新组件（编排层/基座）按仓库惯例带完整文档
注释。后续审计又完成 Dsh 压缩日志峰值内存修复、Codex detail coordinator 拆分和
死代码清理；架构目标基本达成，明细见下。
...
**阶段 9（1c58c79、28940f9）**：修复 Dsh 压缩 session 的通用峰值内存问题。此前压缩输入、
解压后的完整 `Data`、JSONL 解析中间态可能同时驻留进程内存；现在生产路径把 zstd/Node
解压输出流式写入私有临时文件，再用有界 chunk 读取和逐行解析，仍保留单文件失败隔离、
最大行/总输出上限与取消语义。新增跨 chunk/跨行边界回归测试；旧的整块 Data 解压实现
因无调用方一并删除。Dsh 测试 23 个全绿。

**阶段 10（96ce0b5）**：完成 AppState 遗留的 Codex detail task 抽取。新增
`CodexUsageDetailsCoordinator`，集中管理取消、token freshness、scanning 状态和配置代数
回写；AppState 只保留调度入口与状态应用，消除最后一组 detail task 字典/回调环。
既有状态/调度回归 70 个全绿。

**阶段 11（当前审计批次）**：全局源码与测试扫描结论落地。源码未发现可确认的生产死代码、
TODO/FIXME 或未使用的 provider 注册；基座中的抽象 `fatalError` 是强制子类 hook，保留。
删除的仅是阶段 9 后失去调用方的 Dsh buffered decoder。GLM off-peak 文件存在性检查改用
注入的 `FileManagerBox`。测试不做按数量削减：独立取消/过期结果/配置代数/失败恢复/跨
provider 隔离等都代表不同状态边界；仅继续采用 table-driven 合并同形参数矩阵，当前
24 个测试文件、377 个测试方法。

### 遗留建议（后续可选）
1. 以真实设备的大 WAL/锁竞争数据评估 SQLiteTempCopy 的渐进 backup 接口；没有基准
   数据前不改动稳定的临时副本语义。
2. Minimax/Antigravity performScanPure 迁到实例方法并入基座（需重写其静态测试
   表面，预计再 -200 行）。
3. codex JSON 解析风格迁移 AnyJSON（纯风格统一，不带行为或性能收益）。
